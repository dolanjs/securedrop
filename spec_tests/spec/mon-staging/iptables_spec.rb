...
desired_iptables_rules = [
  ...
  '-A INPUT -d 10.0.2.15/32 -i eth0 -p tcp -m tcp --sport 587 -m state --state RELATED,ESTABLISHED -j ACCEPT',
  '-A INPUT -s 10.0.2.15/32 -p udp -m udp --dport 1514 -m state --state RELATED,ESTABLISHED -j ACCEPT',
  "-A INPUT -s #{property['app_ip']}/32 -p udp -m udp --dport 1514 -m state --state RELATED,ESTABLISHED -j ACCEPT",
  '-A INPUT -s 8.8.8.8/32 -d 10.0.2.15/32 -i eth0 -p udp -m udp --sport 53 -m state --state RELATED,ESTABLISHED -j ACCEPT',
  ...
  '-A OUTPUT -s 10.0.2.15/32 -o eth0 -p tcp -m owner --uid-owner 109 -m state --state NEW,RELATED,ESTABLISHED -j ACCEPT',
  ...
  "-A OUTPUT -s 10.0.2.15/32 -d 8.8.8.8/32 -o eth0 -p udp -m udp --dport 53 -m owner --uid-owner #{property['postfix_user_uid']} -m state --state NEW,RELATED,ESTABLISHED -j ACCEPT",
  "-A OUTPUT -s 10.0.2.15/32 -o eth0 -p tcp -m tcp --dport 587 -m owner --uid-owner #{property['postfix_user_uid']} -m state --state NEW,RELATED,ESTABLISHED -j ACCEPT",
  "-A OUTPUT -d #{property['app_ip']}/32 -p udp -m udp --sport 1514 -m owner --uid-owner #{property['ossec_user_uid']} -m state --state RELATED,ESTABLISHED -j ACCEPT",
